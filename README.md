# Generative AI Project Share

This repository contains the minimum set of materials for sharing the current project direction with teammates.

## Included Materials

- `docs/proposal/project-proposal.pdf`: submitted or presentation-ready proposal copy
- `docs/proposal/proposal-summary-latest.md`: latest concise summary of the revised project direction
- `docs/planning/team-roles-and-milestones.md`: team work-package split and milestone plan
- `docs/planning/next-steps.md`: immediate action items for the team

## Current Project Direction

The project is now framed as a focused benchmark study rather than a full end-to-end antibody generation pipeline. The main question is whether pretrained antibody-aware or interaction-aware protein representations can improve prediction performance on biologically relevant antibody tasks such as binding or developability.

## Scope

This share repo intentionally excludes raw data, processed data, code, and internal draft history. It is meant for quick team alignment on proposal direction, division of work, and near-term execution.
